...
const C={
  sky:"#FFD166",      // SUN
  green:"#43C77A",    // FOREST
  violet:"#E0A24B",   // DESERT
  gold:"#BDEBFF",     // ICE
  orange:"#19B5E8",   // OCEAN
  cyan:"#7DE7FF",     // POLAR WATER
  red:"#FF836D",
  muted:"#93AFBE",
  grid:"rgba(189,235,255,.10)"
};
const cfg={displayModeBar:false,responsive:true,scrollZoom:false};
const base={paper_bgcolor:"rgba(0,0,0,0)",plot_bgcolor:"rgba(0,0,0,0)",font:{family:"Inter,Segoe UI,Arial",color:"#a9c0cc",size:9},margin:{l:42,r:12,t:10,b:42},xaxis:{gridcolor:C.grid,zeroline:false,tickfont:{size:8,color:"#82a3b4"}},yaxis:{gridcolor:C.grid,zeroline:false,tickfont:{size:8,color:"#82a3b4"}},legend:{orientation:"h",y:-.18,font:{size:8,color:"#93afbe"}},hoverlabel:{bgcolor:"#0b3040",font:{color:"#fff",size:10}}};
...
const topN=(a,n=10)=>[...a].sort((x,y)=>y.value-x.value).slice(0,n);
...
function renderWorldMap(rows){
 ...
 plot("worldMap",[{
   ...
   colorscale:[[0,"#087CA7"],[.22,"#19B5E8"],[.42,"#BDEBFF"],[.62,"#43C77A"],[.82,"#E0A24B"],[1,"#FFD166"]],
   marker:{line:{color:"#04131e",width:.45}},
   hovertemplate:`%{text}<br>${labels[metric]}: %{z:,.3g}<extra></extra>`,
   colorbar:{title:labels[metric],thickness:8,len:.62,tickfont:{size:8,color:"#8baabc"},titlefont:{size:8,color:"#8baabc"}}
...
function renderOverview(){
 ...
 const gdpRank=topN(r.map(x=>({name:x.country,value:x.gdpT})),10).reverse();
 ...
 ],{xaxis:{...base.xaxis,dtick:2},yaxis:{...base.yaxis,title:"Population (M)",tickformat:".3s"},yaxis2:{overlaying:"y",side:"right",showgrid:false,title:"GDP $T",tickprefix:"$",ticksuffix:"T",tickfont:{size:8,color:"#82a3b4"}},legend:{...base.legend,y:-.2}});
 const rg=Object.entries(group(r,"region")).map(([name,rs])=>({name,value:sum(rs,"gdpT")})).sort((a,b)=>b.value-a.value);
 plot("regionGDP",[{type:"pie",labels:rg.map(x=>x.name),values:rg.map(x=>x.value),hole:.62,marker:{colors:[C.sky,C.violet,C.green,C.gold,C.orange,C.cyan,"#8AA6B5"]},textinfo:"percent",hovertemplate:"%{label}<br>$%{value:.2f}T<extra></extra>"}],{margin:{l:10,r:10,t:4,b:25},legend:{...base.legend,y:-.2}});
...
function renderPopulation(){
 ...
 const growth=r.map(x=>{const b=DB.series.find(z=>z.iso3===x.iso3&&z.year===2017);return{name:x.country,value:deltaPct(x.populationM,b?.populationM||x.populationM)}});const gr=topN(growth,12).reverse();
 ...
 const fr=topN(r.map(x=>({name:x.country,value:x.fertility})),10).reverse();
...
function renderEconomy(){
 ...
 const gr=topN(r.map(x=>({name:x.country,value:x.gdpT})),12).reverse();plot("economyGDP",[{type:"bar",orientation:"h",y:gr.map(x=>x.name),x:gr.map(x=>x.value),marker:{color:C.violet}}],{margin:{l:115,r:10,t:5,b:35},xaxis:{...base.xaxis,tickprefix:"$",ticksuffix:"T"}});
 const pc=topN(r.map(x=>({name:x.country,value:x.gdpPerCapita})),10).reverse();plot("gdpPcRank",[{type:"bar",orientation:"h",y:pc.map(x=>x.name),x:pc.map(x=>x.value),marker:{color:C.gold}}],{margin:{l:110,r:10,t:5,b:35},xaxis:{...base.xaxis,tickprefix:"$",tickformat:".2s"}});
 ...
 const td=topN(r.map(x=>({name:x.country,value:x.tradePctGDP})),10).reverse();plot("tradeRank",[{type:"bar",orientation:"h",y:td.map(x=>x.name),x:td.map(x=>x.value),marker:{color:C.sky}}],{margin:{l:110,r:10,t:5,b:35},xaxis:{...base.xaxis,ticksuffix:"%"}});
...
function renderDevelopment(){
 ...
 const hr=topN(r.map(x=>({name:x.country,value:x.hdi})),12).reverse();plot("hdiTop",[{type:"bar",orientation:"h",y:hr.map(x=>x.name),x:hr.map(x=>x.value),marker:{color:C.gold}}],{margin:{l:115,r:10,t:5,b:35},xaxis:{...base.xaxis,range:[0.4,1]}});
 ...
 ],{xaxis:{...base.xaxis,dtick:2},yaxis:{...base.yaxis,title:"HDI",range:[Math.max(.4,Math.min(...tr.map(x=>x.hdi))-.03),1]},yaxis2:{overlaying:"y",side:"right",showgrid:false,title:"Years",tickfont:{size:8,color:"#82a3b4"}},legend:{...base.legend,y:-.2}})
}
function renderMarkets(){
 ...
 const mr=topN(r.map(x=>({name:x.country,value:x.marketCapT})),12).reverse();plot("marketCapRank",[{type:"bar",orientation:"h",y:mr.map(x=>x.name),x:mr.map(x=>x.value),marker:{color:C.orange}}],{margin:{l:115,r:10,t:5,b:35},xaxis:{...base.xaxis,tickprefix:"$",ticksuffix:"T"}});
 ...
 const cr=topN(cs.map(c=>({name:c.company,value:c.companyMarketCapB,country:c.country})),12).reverse();plot("companyRank",[{type:"bar",orientation:"h",y:cr.map(x=>x.name),x:cr.map(x=>x.value),text:cr.map(x=>x.country),marker:{color:C.violet},hovertemplate:"%{y}<br>%{text}<br>$%{x:.0f}B<extra></extra>"}],{margin:{l:155,r:10,t:5,b:35},xaxis:{...base.xaxis,tickprefix:"$",ticksuffix:"B"}});
 const sectors=Object.entries(group(cs,"companySector")).map(([name,x])=>({name,value:x.length})).sort((a,b)=>b.value-a.value);plot("companySector",[{type:"pie",labels:sectors.map(x=>x.name),values:sectors.map(x=>x.value),hole:.58,marker:{colors:[C.violet,C.sky,C.green,C.gold,C.orange,C.cyan,"#8AA6B5"]},textinfo:"percent"}],{margin:{l:10,r:10,t:3,b:25},legend:{...base.legend,y:-.2}});
 const sr=topN(cs.map(c=>({name:c.country,value:c.sportsScore,medals:c.sportsMedals})),12).reverse();plot("sportsRank",[{type:"bar",orientation:"h",y:sr.map(x=>x.name),x:sr.map(x=>x.value),text:sr.map(x=>`${x.medals} medals`),marker:{color:C.green},hovertemplate:"%{y}<br>Score %{x:.1f}<br>%{text}<extra></extra>"}],{margin:{l:110,r:10,t:5,b:35}});
...
function renderDeepDive(){
 ...
 ],{xaxis:{...base.xaxis,dtick:2},yaxis:{...base.yaxis,title:"Population (M)"},yaxis2:{overlaying:"y",side:"right",showgrid:false,title:"GDP/person & HDI scale",tickfont:{size:8,color:"#82a3b4"}},legend:{...base.legend,y:-.2}});
 ...
 plot("countryCompare",[
  ...
  {type:"bar",name:"Global",x:idx.map(x=>x.name),y:idx.map(x=>x.world),marker:{color:"#6F94A6"}}
